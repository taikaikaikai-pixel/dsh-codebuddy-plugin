...
45. **写完 ≠ 生效完**：POST 同步返回，但网关 `running` 由 `'listening'` 事件异步翻转 ⇒ 紧随的 GET 读到陈旧 false，UI 侧无轮询时假「未监听」永久驻留（保存后按 1s/2s/4s 退避补拉即自愈，card-regression [14] 锁定案）
...
浏览器回归脚本（puppeteer-core + 系统 Chrome，位于仓库外本地目录 `dsh-ui-test/`，不进仓库；2026-08-18 从 `/tmp/dsh-ui-test/` 迁来——/tmp 被系统清空，step9/12/16b/17/18/23 随之丢失；2026-09-03 随设置卡标签页重设计全套重建并换 `/dsh-tap/settings` 新路由；2026-09-19 Windows 侧按 dsh 0.1.6 Plugin Manager 重建 `qoder-slot-check.js`；**2026-09-23 现存清单如下——step 系列与 `_helpers.js` 已在多轮重建中陆续丢失，勿再按名引用**）：`card-regression.js`（**设置卡 28 断言**，接替 step20/22 的不变式：注意条与**独立预言机**双向对齐（strip 缺席时旧写法 `[].every()` 恒真=空洞通过）/8 标签齐全/面板懒挂载与隐藏/模型筛选计数/幽灵输入/HelpNote 折叠展开再收起/用量刷新按钮与时间戳/标签键盘导航（含 roving tabIndex、tabpanel 双向引用、**方向键从聚焦处起算**）/浅色主题/无 pageerror，外加两组 **mock GET/POST** 通道——「启用+未监听 → warn 芯片带端口」正向回归 [12] 与「保存后退避补拉自愈」[14]（踩坑 #45），二者**零真实写入**，跑前跑后对 `~/.dsh/codebuddy-plugin.json` 取哈希对账）、`qoder-slot-check.js`（槽迁移 + Qoder CN 标签 **10 断言**，点击入口必须选卡片 `cardTitle` 按钮而非侧栏会话树同名行）、`qoder-e2e.js`（Qoder 通道端到端：选择器出模→选 qoder/auto→发消息→收回复）、`qoder-prefs-check.js`（Qoder 模型行思考强度/上下文 select）、`qoder-tab-phase2.js`（Qoder CN 标签：启用开关/目录同步/模型启停组/端口行）、`shots-baseline.js`（8 标签明暗基线截图）、`debug-dom.js`/`debug-inputs.js`（宿主原语真实 DOM dump，选择器校准用——踩坑 #44③ 的判别手法）。跑前 `dsh web`，跑后杀 3080。选择器一律按 `.cbc-*` 类与行内单元格精确匹配（踩坑 #16 与 step20 误删 Key 的教训），改 UI 文案/结构后先 grep 旧脚本的选择器；脚本基线一律从 GET /settings 实况读取并收尾复原（含文件层擦除），不硬编码起始模式。
