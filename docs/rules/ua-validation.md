...
> 证据：`docs/probes/ua-2026-08-19.jsonl`（74 条探测记录，含每轮预注册预测与 HIT/MISS 判定）
> 脚本：`scripts/probe-ua.mjs`（`--set matrix|predict…predict12` 可复跑全部轮次）
...
- **`/v2/chat/completions`：无 UA 门**。真实字节证据：`docs/probes/bridge-flash-ab-2026-08-18.jsonl` 中线上桥以 `user-agent: deepseek-harness/0.1.0-rc.6` 成功；探测补证 `Garbage/0.0` + `stream:true` → 200 SSE 流（`predict-chat-stream-garbage-ua` HIT）。
...
```bash
node scripts/probe-ua.mjs --set matrix      # 17 变异 + 2 chat 臂
node scripts/probe-ua.mjs --set predictN    # N=1..12，逐轮预注册预测
```
