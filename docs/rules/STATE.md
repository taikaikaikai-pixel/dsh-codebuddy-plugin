...
4. **用量统计归因修复（Qoder CN"用了不统计"报障闭合，插件侧）**：判别实验定案——裸 OpenAI body 聊天后 quota/heatmap/summary 计数器全不动（高精度哨兵 totalCredits 11 位小数无变化），即**裸推理请求服务端根本不记账**，"能聊天"与"被记账"是两条链路。官方归因形态（bundle A6e/g4i/aPl 原文）：聊天信封归因字段+business 块、收尾 business/finish（mode auth）、/api/v1/tracking（mode sign）。插件网关已全量对齐（verify-qoder [19] 12 断言锁；三模型家族真实上游 200 回归）。新脚本：probe-qoder-quota.mjs（计数器差分）、probe-qoder-attribution.mjs（7 臂梯度）。**待确认**：服务端入库延迟窗口（15 分钟观测不动属正常；已挂 07:37 一次性 cron 长窗口复测——另注意官方 transcript 里 qfmodel 恒 billable:false 免费档，"官方用了没统计"先排除免费档模型）；若届时仍不动则缺 aid/yx_uid（官方取自登录态 userInfo，需重新走一次真实登录捕获 poll 响应全字段）
