...
8. **待办**：①浏览器端回归未跑（`dsh-ui-test/` 的 step20/22/25… 与 `qoder-slot-check.js`）——设置卡在 0.1.7 Plugin Manager 里的渲染与八标签交互需实测一轮；②`0.1.7` 新增的 `icon` / 本地化 `title`+`description` 清单字段可选接入（Plugin Manager 卡片观感），未做；③`bundle.patch` 现可为数组，未来拆分 patch 时可用；④上游 alpha 线仍会变，复跑 `npm pack` 逐包 diff + `verify-host-config` + `?probe=host-config` 三件套即可复检。
