...
| 头部开关误触（收起态直接启用/停用通道） | ~~开关即普通 checkbox、可逆；若实感不佳，回退方案 = 开关移入展开区第一行（已与用户约定）~~ ⇒ **0.13.0 已落地首选方案**：收起态第一次点击只进确认态（就地提示「再点一次确认」+ 受控框弹回 + 4s 超时复位，沿用 askConfirm 模式），展开态直切不确认；回退方案不启用。锁 = card-accordion `[C3]`（首击 0 POST / 二击恰 1 POST / 超时复位 / 展开直切） |
