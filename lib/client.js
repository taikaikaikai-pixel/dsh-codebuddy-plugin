		...
		function BlockHead(props) {
			...
			var kids = [
				createElement("button", {
					...
					// 键盘激活显式化（Task 7 [G1] 回归锁）：原生 button 只对「受信任」
					// 按键事件执行激活行为，脚本派发/部分 AT 派发的 keydown 不会翻转
					// aria-expanded。preventDefault 吃掉浏览器默认的 click 激活，真实
					// Enter 键也只翻转一次（不双翻）；Space 的原生激活行为保持不动。
					onKeyDown: function (e) {
						if (e.key === "Enter") { e.preventDefault(); props.onToggle(); }
					},
				},
