	...
	factory: (require) => {
		...
		function CodeBuddyCard(props) {
			...
			var strip = createElement("div", { className: "cbc-strip" },
				[chips.login, chips.models, chips.bridge, chips.search, chips.image, chips.trae, chips.qoder].map(function (c, i) {
					return createElement("button", {
			...
			return {
				...
				qoder: {
					...
					text: value.qoderEnabled === true
						? "Qoder " + ((((data.qoder || {}).bridge || {}).running === true) ? "运行中" : "网关未监听")
		...
		function tabBadge(data) {
			...
			var qoder = value.qoderEnabled === true
				? (((data.qoder || {}).bridge || {}).running === true ? { text: "运行", tone: "ok" } : { text: "开", tone: null })
					...
					parts.push(createElement("div", { key: "out", className: "cbc-addrow" },
						confirmAct === "__logout__"
							? createElement(CbcButton, { variant: "outline", danger: true, onClick: function () {
								clearConfirm();
								post({ action: "oauth-logout" }).then(reload).catch(function () { setErr("退出失败（网络）"); });
							} }, "确认退出")
				...
				var rows = sortedKeys.map(function (k) {
					...
					return createElement("div", { key: k.name, className: "cbc-listrow" },
						...
						confirmAct === k.name
							? createElement(CbcButton, { variant: "outline", danger: true, onClick: function () {
								clearConfirm();
								post({ patch: { apiKeysRemove: k.name } }).then(reload).catch(function () { setErr("删除失败（网络）"); });
							} }, "确认删除")
		...
		function ModelsSection(props) {
				...
				post({ patch: { modelSetEnabled: payload } }).then(function (res) {
					...
					if (res.d.models) {
						...
						setData(function (prev) {
							...
							return prev ? { catalog: prev.catalog, staticIds: prev.staticIds, staticEfforts: prev.staticEfforts, profiles: prev.profiles, ceilings: prev.ceilings, effectiveIds: res.d.models.effectiveIds || prev.effectiveIds, state: { disabled: dis, extra: ext, overrides: res.d.models.overrides || {} } } : prev;
						});
					}
			...
			if (data && data.catalog) {
				...
				var makeRow = function (m, fromCatalog) {
					...
					// G8：静态行有档位表时出 select 直接设档（存 effortByModel，
					// 桥出站注入 reasoning_effort）；off 不进选项——其线值是
					// null（= 省略参数），与"默认"等价。
					var effortText = null;
					var effortSel = null;
					if (isStatic) {
						var tiers = ((data.staticEfforts || {})[m.id] || []).filter(function (t) { return t !== "off"; });
						if (tiers.length) {
							effortSel = createElement("select", {
								...
								tiers.map(function (t) { return createElement("option", { key: t, value: t }, "思考:" + t); }));
						}
		...
		function ProvidersSection(props) {
			...
			return createElement("div", { className: "cbc-section" },
				...
				createElement("p", { className: "cbc-hint" }, "key 型 OpenAI 兼容上游：添加时实测验证 key（有目录的拉 GET /models，无目录/公开目录的走聊天探针）并把模型写进选择器（免重启）；key 落 ~/.dsh/.credentials.yaml（<ID>_API_KEY，0600），只回脱敏值；删除连同凭据一起清。"));
		}
		...
		function TraeSection(props) {
			...
			var doTLogout = function () {
				...
				post({ action: "trae-oauth-logout" }).then(reload).catch(function () { setErr("退出失败（网络）"); });
			};
		...
		// --- Qoder CN：设备流 OAuth + 翻译网关 + 网关目录模型启停 ----------------
		function QoderSection(props) {
			...
			var doQLogout = function () {
				...
				post({ action: "qoder-oauth-logout" }).then(reload).catch(function () { setErr("退出失败（网络）"); });
			};
			...
			if (value.qoderEnabled === true && qlist && qlist.profiles.length) {
				...
				qmodelRows = createElement("div", { key: "qmodels", style: { marginTop: 4 } },
					...
					createElement("p", { className: "cbc-hint" }, "勾选控制每个 Qoder 模型是否进对话选择器（写入 ~/.dsh/settings.yaml 的 providers.qoder 块，免重启）；全部取消 = 整个 Qoder 路由从选择器移除。"));
			}
			...
			return createElement("div", { className: "cbc-section" },
				...
				createElement("p", { className: "cbc-hint" }, "Qoder CN（qoder.cn）设备流 OAuth + COSY 签名翻译网关：对话经本地 :3903 网关转成官方 infer 协议（凭据只存 ~/.dsh/qoder-plugin-auth.json，不回传浏览器）。额度跟账号走（按 usage.credits 计量，额度与用量页可见）。"));
		}
		...
		function startOAuthFlow(post, action, setBusy, setErr, reload) {
			...
			}).catch(function () { setBusy(false); if (win) win.close(); setErr("发起登录失败（网络）"); });
		}
