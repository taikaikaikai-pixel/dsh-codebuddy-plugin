 ...
 *   2. Expanded: an attention bar that only appears when something needs it
 *      (warn/err chips, click jumps straight to the owning tab) above a
 *      horizontal tab bar whose badges carry the always-on status — the same
 *      tab pattern the host's own settings dialog uses.
 *   3. Tabs (登录 / 模型 / 额度与用量 / 工具 / 服务商 / TraeWork CN / Qoder CN /
 *      桥与高级) lazy-mount on first visit and then stay mounted hidden, so
 *      drafts, scroll positions and fetched catalogs survive both tab
 *      switches and saves. Heavy work stays lazy: model-list / provider-list /
 *      trae model-list load when their tab first opens; the usage poller only
 *      runs while the 额度与用量 tab is active. Long background explanations
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function CodeBuddyCard(props) {
			...
			var openState = useState(embedded);
			...
			var load = function () {
				return fetchWithTimeout(ROUTE, { headers: { accept: "application/json" } }, 20000)
					...
					.then(function (d) { setData(d); setErr(""); return d; })
					.catch(function (e) { setErr(e && e.message ? e.message : "设置服务不可达"); return null; });
			};
			// Mount 即拉一次（折叠态头部芯片要有真值），每次展开（false→true）
			// 再刷新——收起不拉：[open] 依赖在 true→false 时同样 fire。
			var prevOpenRef = useRef(null);
			useEffect(function () {
				if (prevOpenRef.current === null || (open && prevOpenRef.current === false)) load();
			...
			var cardClass = "cbc-card" + ((open || embedded) ? " cbc-open" : "");
			var cardTag = embedded ? "div" : "li";
			if (!open && !embedded) return createElement(cardTag, { className: cardClass }, header);
			if (!data) {
			...
			// 注意条：只浮出需要处理的状态（warn/err），点击直跳所属分区；
			// 常态状态由标签徽标承载——不再重复铺一整排（2026-09 简洁化）。
			var attn = [chips.login, chips.models, chips.bridge, chips.search, chips.image, chips.trae, chips.qoder]
				.filter(function (c) { return c.tone === "warn" || c.tone === "err"; });
			var strip = attn.length
				? createElement("div", { className: "cbc-strip" },
					attn.map(function (c, i) {
						return createElement("button", {
			...
			// 键盘导航：←/→ 从**当前聚焦**的标签起算（不是 activeTab——roving tabIndex
			// 下两者可以不一致），切换并把焦点一起移过去。
			var tabIds = TAB_DEFS.map(function (d) { return d.id; });
			var onTabKeyDown = function (e) {
				if (e.key !== "ArrowRight" && e.key !== "ArrowLeft") return;
				e.preventDefault();
				var idx = tabIds.indexOf(e.currentTarget.getAttribute("data-tab"));
				if (idx < 0) idx = tabIds.indexOf(activeTab);
				var next = (idx + (e.key === "ArrowRight" ? 1 : tabIds.length - 1)) % tabIds.length;
				switchTab(tabIds[next]);
				var btns = e.currentTarget.parentNode.querySelectorAll(".cbc-tab");
				if (btns[next]) btns[next].focus();
			};
			var tabBar = createElement("div", { className: "cbc-tabs", role: "tablist" },
				TAB_DEFS.map(function (def) {
					var b = badge[def.id];
					var selected = activeTab === def.id;
					return createElement("button", {
						key: def.id, type: "button", role: "tab",
						id: "cbc-tab-" + def.id, "data-tab": def.id,
						className: "cbc-tab" + (selected ? " cbc-active" : ""),
						"aria-selected": selected ? "true" : "false",
						// roving tabIndex：只有活跃标签进 Tab 序，键盘用户一次 Tab 即穿过标签栏。
						tabIndex: selected ? 0 : -1,
						// 懒挂载：分区未挂载时没有对应 tabpanel，不留空引用。
						"aria-controls": mounted[def.id] ? "cbc-panel-" + def.id : undefined,
			...
			var panelProps = {
				...
				models: { post: post, setErr: setErr, reload: load, modelsInfo: data.models || {}, value: data.value || {}, save: save },
				...
				qoder: { value: data.value || {}, save: save, post: post, reload: load, setErr: setErr, overridden: overriddenFor(data), qoder: data.qoder || {} },
			...
			var panels = TAB_DEFS.map(function (def) {
				...
				return createElement("div", {
					...
					id: "cbc-panel-" + def.id, role: "tabpanel",
					"aria-labelledby": "cbc-tab-" + def.id,
			...
			return {
				...
				trae: {
					...
					text: value.traeEnabled === true
						? "Trae " + ((((data.trae || {}).bridge || {}).running === true) ? "运行中" : "网关未监听 :" + (value.traeBridgePort || "?"))
						: "Trae 未启用",
				},
				qoder: {
					tab: "qoder", tabTitle: "Qoder CN",
					...
					text: value.qoderEnabled === true
						? "Qoder " + ((((data.qoder || {}).bridge || {}).running === true) ? "运行中" : "网关未监听 :" + (value.qoderBridgePort || "?"))
						: "Qoder " + (qoauth.signedIn ? (qoauth.needsRelogin ? "需重登" : "已登录·未启用") : (qoauth.pending ? "登录中" : "未启用")),
				},
			};
		}

		// 标签徽标：数字/短词来自 GET 视图（usage 等懒加载数据不上徽标）。
		// 常态状态的唯一承载处（注意条只浮出 warn/err）——启用但网关未监听算 warn。
		function tabBadge(data) {
			...
			var login = value.authMode === "oauth"
				? (oauth.needsRelogin ? { text: "需重新登录", tone: "err" } : oauth.signedIn ? { text: "已登录", tone: "ok" } : { text: "未登录", tone: "warn" })
				: null;
			var toolsOn = (value.searchEnabled === true ? 1 : 0) + (value.imageGenEnabled === true ? 1 : 0);
			var tools = { text: toolsOn + "/2", tone: toolsOn === 2 ? "ok" : null };
			var trae = value.traeEnabled === true
				? ((data.trae || {}).bridge || {}).running === true ? { text: "运行", tone: "ok" } : { text: "未监听", tone: "warn" }
				: null;
			var qoauth = (data.qoder || {}).oauth || {};
			var qoder = value.qoderEnabled === true
				? (((data.qoder || {}).bridge || {}).running === true ? { text: "运行", tone: "ok" } : { text: "未监听", tone: "warn" })
				: (qoauth.signedIn ? (qoauth.needsRelogin ? { text: "需重登", tone: "err" } : { text: "已登录", tone: "ok" }) : (qoauth.pending ? { text: "登录中", tone: "warn" } : null));
			return {
				login: login,
		...
		function UsageSection(props) {
			...
			rows.push(createElement("div", { key: "upd", className: "cbc-row" },
				...
				createElement("div", { className: "cbc-row-control" },
					...
					createElement("span", { className: "cbc-updated" }, lastUpd ? "更新于 " + fmtClock(lastUpd) + " · 本页可见时每 10 秒自动刷新" : "正在读取…"))));
		...
		function ModelsSection(props) {
			...
			var fetchList = function () {
				...
				}).catch(function (e) { setBusy(false); setErr(e && e.message ? e.message : "获取失败（网络）"); });
			};
			...
			if (data && data.catalog) {
				...
				body = createElement("div", { style: { marginTop: 8 } },
					createElement("div", { className: "cbc-scrollbox", style: { maxHeight: 400 } },
						createElement("p", { className: "cbc-group-title" }, needle
		...
		function QoderSection(props) {
			...
			if (value.qoderEnabled === true && qlist && qlist.profiles.length) {
				...
				qmodelRows = createElement("div", { key: "qmodels", style: { marginTop: 4 } },
					createElement("div", { className: "cbc-group-title" }, "Qoder 模型（选择器内 " + enabledCount + " / " + qlist.profiles.length + "）"),
					createElement("div", { className: "cbc-scrollbox", style: { maxHeight: 220 } },
						qlist.profiles.map(function (p) {
			...
			return createElement("div", { className: "cbc-section" },
				...
				createElement("div", { className: "cbc-row" },
					createElement("div", { className: "cbc-row-label" }, "启用通道"),
					createElement("div", { className: "cbc-row-control" },
						createElement("input", { type: "checkbox", className: "cbc-check", checked: value.qoderEnabled === true, onChange: function (e) { save({ qoderEnabled: e.target.checked }); } }),
						createElement("span", { className: "cbc-muted" }, value.qoderEnabled === true
							? "翻译网关已上线（:" + value.qoderBridgePort + (qbridge.running ? "，运行中" : "，未监听" + (qbridge.lastError ? "：" + qbridge.lastError : "")) + "）"
							: "已禁用（选择器里的 Qoder 模型会整体移除）"),
						createElement(ResetButton, { fieldKey: "qoderEnabled", overridden: overridden("qoderEnabled"), save: save }))),
				createElement("div", { className: "cbc-row" },
					createElement("div", { className: "cbc-row-label" }, "模型目录"),
					createElement("div", { className: "cbc-row-control" },
						createElement(CbcButton, { variant: "outline", disabled: busy, onClick: syncModels }, busy ? "同步中…" : "从网关同步"),
						createElement("span", { className: "cbc-muted" }, syncInfo))),
				qmodelRows,
				createElement("div", { className: "cbc-row" },
					createElement("div", { className: "cbc-row-label" }, "网关端口"),
				...
				createElement("div", { className: "cbc-row" },
					...
					createElement("div", { className: "cbc-row-control" },
						createElement("button", { type: "button", className: "cbc-toggle", onClick: function () { setAdvOpen(!advOpen); } },
							advOpen ? "▾ 收起高级连接设置" : "▸ 登录域 / OpenAPI / infer / client_id（高级）"))),
				advOpen ? createElement("div", { className: "cbc-row" },
					createElement("div", { className: "cbc-row-label" }, ""),
					createElement("div", { className: "cbc-row-control" },
						...
						createElement(TextField, { fieldKey: "qoderClientId", value: value.qoderClientId, save: save, widthClass: "cbc-w220", overridden: overridden("qoderClientId") }))) : null,
		...
		PANEL_RENDERERS.qoder = QoderSection;
		...
		function BridgeAdvancedSection(props) {
			...
			return createElement("div", { className: "cbc-section" },
				...
				createElement(HelpNote, null, "桥统一解析凭据（登录区选 OAuth 或 Key），主聊天也经由此桥。同一会话超过上限的请求排队（FIFO）；无会话 id 的请求不限流。非流式入站聚合成标准 JSON，chat/completions 之外的请求直接透传。"),
				createElement("hr", { className: "cbc-divider" }),
