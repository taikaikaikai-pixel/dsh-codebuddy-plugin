	...
	factory: (require) => {
		...
		function ModelsSection(props) {
			...
			return createElement("div", { className: "cbc-section" },
				...
				body,
				createElement("p", { className: "cbc-hint" }, "启动时自动同步网关目录（/v3/config）并入对话选择器，此处可手动再同步；勾选控制每个模型是否出现在选择器，ctx/输出上限可直接改（清空恢复目录默认），均写入宿主配置层（dsh 0.1.7+ 落 profile 的 cordis.patch.yml，更早版本落 ~/.dsh/settings.yaml 的 llm-pi-ai 覆盖层），下次请求生效。"));
		}
		...
		function TraeSection(props) {
			...
			if (value.traeEnabled === true && tlist && tlist.profiles.length) {
				...
				tmodelRows = createElement("div", { key: "tmodels", style: { marginTop: 4 } },
					...
					createElement("p", { className: "cbc-hint" }, "勾选控制每个 Trae 模型是否进对话选择器（写入宿主配置层的 providers.trae 块——dsh 0.1.7+ 落 profile patch，更早落 ~/.dsh/settings.yaml；免重启）；全部取消 = 整个 Trae 路由从选择器移除。"));
			}
