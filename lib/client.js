 ...
 *      流式桥), so the card is readable without expanding (transitional layer,
 *      retired once status converges onto the block heads).
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 分区 → 所属区块（注意条芯片的跳转目标；strip 随 Task 2 删除后此表可一并清掉）。
		var SECTION_BLOCK = {};
		BLOCK_DEFS.forEach(function (def) {
			BLOCK_SECTIONS[def.id].forEach(function (sid) { SECTION_BLOCK[sid] = def.id; });
		});
		...
		function CodeBuddyCard(props) {
			...
			// 幂等展开：注意条（Task 2 删）点击是「带我去」语义，已展开的区块不该被关掉。
			...
			// 注意条（Task 2 删除）：只浮出需要处理的状态（warn/err）；点击展开所属区块。
				...
				? createElement("div", { className: "cbc-strip" },
					attn.map(function (c, i) {
						return createElement("button", {
							...
							onClick: function () { showBlock(SECTION_BLOCK[c.tab] || "codebuddy"); },
		...
		function UsageSection(props) {
			var post = props.post;
			var value = props.value || {};
			var active = props.active !== false; // active 跟随「通用」区块开合（sectionProps.usage.active = !!openBlocks.general）
