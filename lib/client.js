...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function ModelsSection(props) {
			...
			// busy/setBusy 只服务首次挂载拉取，这里用独立的 syncBusy。
		...
		function QoderSection(props) {
			...
			if (value.qoderEnabled === true && qlist && qlist.profiles.length) {
				...
				var qneedle = qfilter.trim().toLowerCase();
				var qshown = qlist.profiles.filter(function (p) {
					if (!qneedle) return true;
					return String(p.id).toLowerCase().indexOf(qneedle) >= 0
						|| (p.name && String(p.name).toLowerCase().indexOf(qneedle) >= 0);
				});
