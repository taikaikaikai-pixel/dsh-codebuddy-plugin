 ...
 *   GET  → { value, user, oauth, bridge, models, trae }
	...
	factory: (require) => {
			...
			if (value.authMode === "oauth") {
				...
				if (oauth.signedIn && !oauth.needsRelogin) {
					...
					parts.push(createElement("div", { key: "out", className: "cbc-addrow" },
						createElement(CbcButton, { variant: "outline", danger: true, onClick: function () { post({ action: "oauth-logout" }).then(reload).catch(function () { setErr("退出失败（网络）"); }); } }, "退出登录")));
				...
				var rows = sortedKeys.map(function (k) {
					...
					return createElement("div", { key: k.name, className: "cbc-listrow" },
						...
						createElement(CbcButton, { variant: "ghost", danger: true, onClick: function () { post({ patch: { apiKeysRemove: k.name } }).then(reload).catch(function () { setErr("删除失败（网络）"); }); } }, "删除"));
				});
				body = rows.concat([
		...
		function TraeSection(props) {
			...
			return createElement("div", { className: "cbc-section" },
				...
				createElement("div", { className: "cbc-row" },
					...
					createElement("div", { className: "cbc-row-control" },
						toauth.signedIn
							? createElement(CbcButton, { variant: "outline", danger: true, onClick: function () { post({ action: "trae-oauth-logout" }).then(reload).catch(function () { setErr("退出失败（网络）"); }); } }, "退出登录")
		...
		function apply(ctx) {
			ctx.inject(["slots"], function (sctx) {
				// dsh ≥ 0.1.5 起槽位由设置页运行时声明（条目 children 链：
				// settings.section → settings.plugins.tab → settings.plugin.item），
				// 直接 register 会抢跑在声明之前——经 slots.inject 等声明落地再
				// 注册（官方内置卡与 dshmarket 同形态）。老版本没有 slots.inject，
				// 退回直接注册（带 rc.6 列表槽需要的 id/order/label）。
